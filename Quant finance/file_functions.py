# -*- coding: utf-8 -*-
"""
Created on Tue Nov 15 18:18:05 2022
...
def load_timeseries(ric):
    ...
    directory = 'C:\\Users\\ilarr\\OneDrive\\Escritorio\\CFA\\TP\\Activos\\'  # hardcoded
...
def table(rics, notional, table=None):
    ...
    port_min_var = {'returns':port1.return_annual, 'sharp':port1.sharpe_annual, 'volatilities':port1.volatility_annual, 'type':'min-variance'}
    port_pca={'returns':port2.return_annual, 'sharp':port2.sharpe_annual, 'volatilities':port2.volatility_annual, 'type':'pca'}
    port_equal={'returns':port3.return_annual, 'sharp':port3.sharpe_annual, 'volatilities':port3.volatility_annual, 'type':'equi-weigh'}
    port_vol_weigh={'returns':port4.return_annual, 'sharp':port4.sharpe_annual, 'volatilities':port4.volatility_annual, 'type':'volatility-weighted'}
    port_long = {'returns':port5.return_annual, 'sharp':port5.sharpe_annual, 'volatilities':port5.volatility_annual, 'type':'long-only'}
    
    if table == 'yes':
        table = pd.DataFrame()
        table['returns']= list(returns.reshape(100,1))
        table['sharp'] = list(sharp)
        table['volatilities']  = volatilities.tolist()
        table['type'] = ['Markowtiz ' + str(returns[j]) for j in range(0,len(returns))]
       
        
        table = table.append(port_min_var, ignore_index=True)
        table = table.append(port_pca, ignore_index=True)
        table = table.append(port_equal, ignore_index=True)
        table = table.append(port_vol_weigh, ignore_index=True)
        table = table.append(port_long, ignore_index=True)
        table = table.sort_values(by=['sharp'], ascending=False)
...
def compute_efficient_frontier(rics, notional, target_return, include_min_variance):
    ...
    #port9 = port_mgr.compute_portfolio('markowitz', target_return)
    #port10 = port_mgr.compute_portfolio('markowitz', target_return)
